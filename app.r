...
server <- function(input, output, session) {
  ...
  output$copula_criteria <- renderPrint({
    req(copula_results())
    fit <- copula_results()
    # AIC: guard against non-numeric or NULL return from AIC()
    aic_val <- tryCatch({
      a <- AIC(fit)
      if (is.numeric(a) && length(a) == 1) round(a, 4) else "N/A"
    }, error = function(e) "N/A")
    cat("AIC: ", aic_val, "\n")
    # edf is a named numeric vector; sum it for total EDF
    edf_val <- tryCatch({
      e <- fit$edf
      if (is.numeric(e)) round(sum(e, na.rm = TRUE), 4) else "N/A"
    }, error = function(e) "N/A")
    cat("Effective degrees of freedom (total): ", edf_val, "\n")
    cat("Sample size (n): ", fit$n, "\n")
    # Additional model selection criteria
    if (!is.null(fit$logLik)) {
      cat("Log-likelihood: ", round(as.numeric(fit$logLik), 4), "\n")
    }
  })
  ...
  output$copula_diag_plot <- renderPlot({
    ...
    # Keep as character so switch() cases match correctly (was incorrectly coerced to numeric)
    eq_char <- input$copula_diag_eq
    gam_obj <- switch(eq_char,
    ...
    if (input$copula_diag_type == "smooth") {
      # Fix: plot.gam internally calls termplot() which uses eval(object$call$data)
      # In Shiny, call$data references a reactive (closure), causing "invalid envir" error.
      # Replace call$data with the stored model frame so termplot gets a real data.frame.
      gam_plot <- gam_obj
      gam_plot$call$data <- gam_obj$model
      tryCatch({
        plot(gam_plot, pages = 1, seWithMean = TRUE, shade = TRUE,
             shade.col = "lightblue", scale = 0, rug = TRUE,
             main = paste("Smooth Terms - Equation", eq_char))
      }, error = function(e) {
        plot.new()
        text(0.5, 0.5, paste("Smooth plot unavailable:\n", conditionMessage(e)),
      ...
      plot(fitval, res, xlab = "Fitted Values", ylab = "Residuals",
           main = paste("Residuals vs Fitted - Equation", eq_char),
           pch = 16, col = "steelblue")
      abline(h = 0, lty = 2, col = "red", lwd = 2)
    } else if (input$copula_diag_type == "qq") {
      qqnorm(residuals(gam_obj), main = paste("Normal Q-Q Plot - Equation", eq_char))
  ...
  output$generate_report <- downloadHandler(
    ...
    content = function(file) {
      withProgress(message = "Generating report...", value = 0, {
        ...
        # ── helpers ──────────────────────────────────────────────────────────
        h  <- function(level, txt) sprintf("<h%d>%s</h%d>", level, txt, level)
        p  <- function(txt)        sprintf("<p>%s</p>", txt)
        hr <- function()           "<hr/>"
        pre <- function(txt)       sprintf("<pre>%s</pre>", htmlspecialchars(txt))
        ...
        tbl_html <- function(df, max_rows = 20) {
          df <- head(df, max_rows)
          header <- paste0("<th>", htmlspecialchars(names(df)), "</th>", collapse = "")
          rows   <- apply(df, 1, function(r) {
            paste0("<tr><td>",
                   paste(htmlspecialchars(as.character(r)), collapse = "</td><td>"),
                   "</td></tr>")
          })
          paste0('<table border="1" cellpadding="4" cellspacing="0" style="border-collapse:collapse;font-size:13px;">',
        ...
        # ── 1. Title & metadata ───────────────────────────────────────────
        setProgress(0.1, detail = "Title block...")
        sections <- c(sections,
          h(1, "Platform Analysis Report"),
          p(paste("Generated:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"))),
          hr())

        # ── 2. Processed data summary ─────────────────────────────────────
        setProgress(0.2, detail = "Data summary...")
        df <- tryCatch(processed_data(), error = function(e) NULL)
        if (!is.null(df)) {
          sections <- c(sections, h(2, "1. Processed Data Summary"))
          sections <- c(sections, p(paste("Observations:", nrow(df),
                                          "| Variables:", ncol(df))))
          sm <- tryCatch({
            sm_txt <- capture.output(summary(df))
            pre(paste(sm_txt, collapse = "\n"))
          }, error = function(e) p("Summary not available."))
          sections <- c(sections, sm)
          sections <- c(sections, h(3, "Variable Overview (first 20 rows)"))
          sections <- c(sections, tbl_html(df[, 1:min(8, ncol(df))]))
          sections <- c(sections, hr())
        }

        # ── 3. Copula model results ────────────────────────────────────────
        setProgress(0.4, detail = "Copula results...")
        fit <- tryCatch(copula_results(), error = function(e) NULL)
        if (!is.null(fit)) {
          sections <- c(sections, h(2, "2. Copula Modeling Results (GJRM)"))

          # Model selection criteria
          aic_val <- tryCatch({
            a <- AIC(fit); if (is.numeric(a)) round(a, 4) else "N/A"
          }, error = function(e) "N/A")
          edf_val <- tryCatch({
            round(sum(fit$edf, na.rm = TRUE), 4)
          }, error = function(e) "N/A")
          ll_val  <- tryCatch({
            round(as.numeric(fit$logLik), 4)
          }, error = function(e) "N/A")
          sections <- c(sections,
            p(paste0("<strong>AIC:</strong> ", aic_val,
                     " &nbsp;|&nbsp; <strong>Log-likelihood:</strong> ", ll_val,
                     " &nbsp;|&nbsp; <strong>Total EDF:</strong> ", edf_val,
                     " &nbsp;|&nbsp; <strong>n:</strong> ", fit$n)))

          # Copula/margin info
          sections <- c(sections,
            p(paste0("<strong>Copula:</strong> ", fit$BivD,
                     " &nbsp;|&nbsp; <strong>Margins:</strong> ",
                     paste(fit$margins, collapse = ", "))))

          # Association parameters
          theta_txt <- tryCatch({
            if (!is.null(fit$theta) && length(fit$theta) > 0) {
              paste(capture.output(print(round(fit$theta, 4))), collapse = "\n")
            } else if (!is.null(fit$theta.a)) {
              paste(capture.output(print(round(fit$theta.a, 4))), collapse = "\n")
            } else { "Smoothly modeled or unavailable." }
          }, error = function(e) "Unavailable.")
          sections <- c(sections,
            h(3, "Association Parameter(s)"),
            pre(theta_txt))
          ...
          sections <- c(sections, h(3, "Model Summary"), pre(sum_txt), hr())
        }

        # ── 4. Strategy results ───────────────────────────────────────────
        setProgress(0.6, detail = "Strategy results...")
        strat <- tryCatch(strategy_results(), error = function(e) NULL)
        if (!is.null(strat)) {
          sections <- c(sections, h(2, "3. Strategy Analysis Results"))
          if (!is.null(strat$agg)) {
            sections <- c(sections, h(3, "Aggregate Outcomes"), tbl_html(strat$agg))
          }
          strat_txt <- tryCatch(
            paste(capture.output(print(strat)), collapse = "\n"),
            error = function(e) "Details not available.")
          sections <- c(sections, pre(strat_txt), hr())
        }

        # ── 5. Causal results ─────────────────────────────────────────────
        setProgress(0.75, detail = "Causal results...")
        causal <- tryCatch(causal_results(), error = function(e) NULL)
        if (!is.null(causal)) {
          sections <- c(sections, h(2, "4. Causal Analysis Results"))
          if (!is.null(causal$ate)) {
            sections <- c(sections, h(3, "Average Treatment Effects"), tbl_html(causal$ate))
          }
          causal_txt <- tryCatch(
            paste(capture.output(print(causal)), collapse = "\n"),
            error = function(e) "Details not available.")
          sections <- c(sections, pre(causal_txt), hr())
        }

        # ── 6. Analysis log ───────────────────────────────────────────────
        setProgress(0.9, detail = "Analysis log...")
        log_df <- tryCatch(analysis_log(), error = function(e) NULL)
        if (!is.null(log_df) && nrow(log_df) > 0) {
          sections <- c(sections, h(2, "5. Analysis Log"), tbl_html(log_df))
        }
        ...
        html_out <- paste0(
          ...
          "h1{color:#1a5276;}h2{color:#1f618d;border-bottom:2px solid #aed6f1;padding-bottom:4px;}",
          "h3{color:#2874a6;}pre{background:#f4f4f4;padding:10px;overflow-x:auto;font-size:12px;}",
          "table{margin-top:8px;margin-bottom:16px;}th{background:#2874a6;color:#fff;}",
          "td,th{padding:5px 10px;}tr:nth-child(even){background:#eaf4fb;}",
          "</style></head><body>",
          paste(sections, collapse = "\n"),
          "<hr/><p style='color:#888;font-size:11px;'>",
          "Report generated by DBA Course Platform &bull; ",
